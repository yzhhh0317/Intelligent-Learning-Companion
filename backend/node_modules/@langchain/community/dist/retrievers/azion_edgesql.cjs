...
exports.AzionRetriever = void 0;
const sql_1 = require("azion/sql");
 ...
 * ```
 */
class AzionRetriever extends retrievers_1.BaseRetriever {
    static lc_name() {
    ...
    async similaritySearchWithScore(query) {
        ...
        const { data: response, error: errorQuery } = await (0, sql_1.useQuery)(this.dbName, statements);
        if (!response) {
    ...
    async extractEntities(query) {
        ...
        const entityExtractionPrompt = new messages_1.SystemMessage(this.promptEntityExtractor);
