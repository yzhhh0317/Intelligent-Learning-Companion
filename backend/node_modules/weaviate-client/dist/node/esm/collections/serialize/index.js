        ...
        if (!args.supportsVectorsForTargets) {
            ...
            Object.entries(args.vector).forEach(([k, v]) => {
                ...
                if (NearVectorInputGuards.isListOf1D(v) || NearVectorInputGuards.isListOf2D(v)) {
                    throw new WeaviateUnsupportedFeatureError('Lists of vectors are not supported in Weaviate <1.29.0');
                }
...
Serialize.targets = (targets, supportsWeightsForTargets) => {
    ...
    if (targets.weights !== undefined && supportsWeightsForTargets) {
        ...
        if (Object.values(targets.weights).some((v) => Array.isArray(v))) {
            throw new WeaviateUnsupportedFeatureError('Multiple weights per target are not supported in this Weaviate version. Please upgrade to at least Weaviate 1.27.0.');
        }
        return {
            combination,
